...
public:
  ...
  T& Get() {
    static const uit::WarnOnce warning{
      "Calling non-const Get on VectorBlockIrecvDuct incurs unnecessary copy."
    };
    //TODO tidy up
    const uit::CircularIndex<N> idx = receive_position - 1;
    cache = buffer[idx];
  ...
  static std::string GetName() { return "VectorBlockIrecvDuct"; }
