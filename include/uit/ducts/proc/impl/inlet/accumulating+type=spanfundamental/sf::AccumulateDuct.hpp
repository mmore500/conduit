...
private:
  ...
  void DoAccumulate(const bundle_t& bundle) {

    // TODO FIXME what kind of lock is needed here?
    back_end->GetWindowManager().LockShared(
      address.GetOutletProc()
    );

    back_end->GetWindowManager().template Accumulate<value_type>(
      address.GetOutletProc(),
...
public:
  ...
  template<typename P>
  bool TryPut(P&& val) {
    DoAccumulate( bundle_t( std::forward<P>(val) ) );
    return true;
  }
