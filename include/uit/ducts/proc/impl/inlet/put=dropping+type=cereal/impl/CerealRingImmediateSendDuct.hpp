...
private:
  ...
  using buffer_t = uitsl::RingBuffer<
    std::tuple<uit_emp::ContiguousStream, uitsl::Request>,
    N
  >;
  buffer_t buffer{};
  ...
  void PostSendRequest() {
    ...
    ImmediateSendFunctor{}(
      std::get<uit_emp::ContiguousStream>( buffer.GetHead() ).GetData(),
      std::get<uit_emp::ContiguousStream>( buffer.GetHead() ).GetSize(),
      MPI_BYTE,
  ...
  void DoPut(const T& val) {
    ...
    { // oarchive flushes on destruction
      std::get<uit_emp::ContiguousStream>( buffer.GetHead() ).Reset();
      cereal::BinaryOutputArchive oarchive(
        std::get<uit_emp::ContiguousStream>( buffer.GetHead() )
      );
      oarchive(val);
    }
    PostSendRequest();
